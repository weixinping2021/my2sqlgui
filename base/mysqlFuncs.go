...
func (this *TablesColumnsInfo) GetTbDefFromDb(cfg *ConfCmd, dbname string, tbname string) {
	...
	if cfg.Mode == "file" {
		this.GetTableColumns_wxp(cfg.TableDef, dbname, tbname)
	} else {
...
func (this *TablesColumnsInfo) GetTableColumns_wxp(tb_sql string, dbname string, tbname string) error {
	...
	//fmt.Print(columns, dbTbFieldsInfo)

	sql_text := tb_sql
	start_index := strings.Index(sql_text, "(")
	end_index := strings.LastIndex(sql_text, ")")
	//fmt.Println(start_index, end_index)
	cut_str := sql_text[start_index+1 : end_index]
	//fmt.Println(cut_str)
	column_values := strings.Split(cut_str, ",")
	for i, value := range column_values {
		fmt.Println(value)
		value = strings.Replace(value, "\n", "", -1)
		co := strings.Fields(value)
		fmt.Println(i, co, co[0])
		if co[0] == "PRIMARY" {
			continue
		}
		...
		dbTbFieldsInfo[tbKey] = append(dbTbFieldsInfo[tbKey], FieldInfo{FieldName: string(co[0]), FieldType: GetFiledType(string(co[1])), IsUnsigned: IsUnsigned("false")})
	}
