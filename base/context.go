...
//work_dir.Text, binlog_select.Selected, db_entry.Text, tb_name_entry.Text, tb_entry.Text, work_type.Selected, sql_types.Selected
...
// work_dir.Text,binlog_entry.Text,db_entry.Text,tb_entry.Text,work_type_Radio.Selected,sql_types.Selected
func (this *ConfCmd) ParseCmdOptions(starttime string, stoptime string) {
	var (
		dbs       string
		tbs       string
		ignoreDbs string
	...
	//flag.BoolVar(&this.PrintExtraInfo, "add-extraInfo", true, "Works with -work-type=2sql|rollback. Print database/table/datetime/binlogposition...info on the line before sql, default false")
	...
	//不需要命令行参数,所以直接注释这个
	//flag.Parse()
	this.MysqlType = "mysql"
	ignoreDbs = ""
	ignoreTbs = ""
	this.StartPos = 4
	this.BinlogTimeLocation = "Local"
	startTime = starttime
	stopTime = stoptime
	this.OutputToScreen = false
	this.PrintExtraInfo = true
	this.FullColumns = false
	doNotAddPrifixDb = false
	this.FilePerTable = false
	this.PrintInterval = this.GetDefaultValueOfRange("PrintInterval")
